    ...
    def effect(self, opponent_card, player, opponent, arg=None):
        ...
        if len(player.hand) > 0:
            for i in range(len(player.hand)):
                now_power = player.hand[i].power(opponent_card)
                if now_power > max_power:
                    max_power = now_power
                    attack = player.hand[i].attack
                    defense = player.hand[i].defense
                    index = i
...
class InstructorCard(Card):
    ...
    def effect(self, opponent_card, player, opponent, arg=None):
        ...
        if self.attack>0 and self.defense>0:
            self.attack-=1000
            self.defense-=1000
            player.hand.append(InstructorCard(self.name, self.attack, self.defense))
            re_add = True
        else :
            self.attack-=1000
            self.defense-=1000
